...
export const setupQueryPersistence = async (
  ...
  try {
    ...
    if (!persisterPkg?.createAsyncStoragePersister || !persistPkg?.persistQueryClient) {
      console.warn("[QueryClient] Persistence packages not fully available, skipping persistence setup");
      return;
    }
    ...
    if (error?.code === "MODULE_NOT_FOUND") {
      console.warn("[QueryClient] Persistence packages not installed. Install @tanstack/query-async-storage-persister and @tanstack/react-query-persist-client for offline support.");
    } else {
