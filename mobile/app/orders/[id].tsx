...
export default function OrderDetailsScreen() {
  ...
  const order = orders.find((o: OrderInterface) => 
    o.documentId === id || String(o.id) === id
  );
          ...
          return (
            <View key={step} style={styles.timelineStep}>
              <View style={[styles.timelineDot, { backgroundColor: stepColor }]}>
                {isCurrent && <View style={[styles.timelineInnerDot, { backgroundColor: "#fff" }]} />}
              </View>
              {index < statusSteps.length - 1 && (
                <View style={[styles.timelineLine, { backgroundColor: isActive ? stepColor : "#d1d5db" }]} />
              )}
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.header}>
          <IconButton icon="arrow-left" size={24} onPress={() => router.back()} />
          <Text variant="headlineSmall" style={styles.headerTitle}>Order Details</Text>
          <View style={{ width: 48 }} />
        </View>
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
          ...
          <Surface style={styles.card}>
            ...
            <View style={styles.priceRow}><Text>Subtotal</Text><Text>₹{order.subtotal?.toFixed(0) || order.totalPrice?.toFixed(0)}</Text></View>
            <View style={styles.priceRow}><Text>Delivery Fee</Text><Text>₹{order.deliveryFee || 0}</Text></View>
            <View style={styles.priceRow}><Text>Platform Fee</Text><Text>₹{order.platformFee || 0}</Text></View>
            <View style={styles.priceRow}><Text>Packaging Fee</Text><Text>₹{order.packagingFee || 0}</Text></View>
            <Divider style={{ marginVertical: 8 }} />
            <View style={[styles.priceRow, styles.totalRow]}>
              <Text variant="titleMedium">Total</Text>
              <Text variant="titleMedium" style={{ color: primaryColor }}>₹{order.totalPrice?.toFixed(0)}</Text>
            </View>
          </Surface>
...
const styles = StyleSheet.create({
  ...
  card: { margin: 16, marginBottom: 12, padding: 16, borderRadius: 12, backgroundColor: "#fff", elevation: 1 },
  orderHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  dateText: { color: "#666", marginTop: 4 },
  deliveryText: { color: "#10b981", marginTop: 4, fontWeight: "500" },
  sectionTitle: { fontWeight: "600", marginBottom: 12 },
  timelineContainer: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingVertical: 12 },
